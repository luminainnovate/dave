...
def call_llm(client: httpx.Client, model: str, system_prompt: str, user_content: str, prior_context: str = "") -> str:
    ...
    print(f"    ↳ All parts finished. Starting Merge Pass...", flush=True)
...
def assemble_clinerules(results: dict, config: dict) -> str:
    ...
    doc = [
        ...
        "",
        f"> Auto-generated by Multi-Agent Distillation Pipeline",
    ...
    doc.extend([
        "## Constraints",
        "",
        f"- Max project size: {limits.get('max_project_size_mb', 2048)} MB",
        f"- Max build iterations: {limits.get('max_build_iterations', 5)}",
        f"- You must verify each major component after implementation",
        f"- Run all safety checks before declaring the build complete",
        "",
    ])
